...
This is done by shipping GraalVM reachability metadata in the `META-INF/native-image` directory. The agent generates
several configuration files, each covering a different aspect of the runtime:

* `reflect-config.json` – classes, methods and fields accessed via reflection
* `resource-config.json` – resources loaded at runtime (including locale `.yml` files)
* `jni-config.json` – JNI-reachable classes, methods and fields
* `proxy-config.json` – dynamic proxy interface lists
* `serialization-config.json` – serialization types
* `predefined-classes-config.json` – classes generated/loaded at runtime

These files can be regenerated by running tests with the GraalVM native-image agent using the provided Maven profile:
...
and merge the metadata files into `src/main/resources/META-INF/native-image`. The profile uses `config-merge-dir`
so existing hand-crafted configuration (e.g. `reachability-metadata.json`) is preserved. Review the resulting diff
before committing and drop any entries that are specific to the test runtime (JUnit, Surefire, Mockito, ByteBuddy).
