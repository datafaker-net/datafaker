...
class PeselNumberTest {
  ...
  void testGenderedPesel(Gender givenGender) {
     ...
     * Given
     */
    final LocalDate givenBirthDate =
        Faker.instance()
            .date()
            .birthday(0, 100)
            .toInstant()
  ...
  void testInvalidCenturiesPesel(int givenBirthYear) {
    ...
    Assertions.assertThrows(
        IllegalArgumentException.class, () -> peselNumber.get(givenBirthDate, Gender.ANY));
  }
  ...
  private void assertControlDigit(String gotPesel) {
    final int gotSum =
        (gotPesel.charAt(0)
                ...
                + 7 * (gotPesel.charAt(2) + gotPesel.charAt(6))
                + 9 * (gotPesel.charAt(3) + gotPesel.charAt(7)))
            % 10;
